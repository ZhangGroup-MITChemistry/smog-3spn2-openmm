...
"""
Rerun trajectory with ff3SPN2 to compare energy. 
"""
...
dsDNA.periodic = True
#dsDNA.template_atoms.to_csv('ff3SPN2_cg_dna_template.csv', index=False)

s = ff3SPN2.System(dsDNA, periodicBox=[box_a, box_b, box_c])
s.add3SPN2forces(verbose=True)
    
#with open(f'ff3SPN2_{dna_type}_system.xml', 'w') as f:
...
df_energies.round(4).to_csv(f'ff3SPN2_{dna_type}_energy.csv', index=False)
